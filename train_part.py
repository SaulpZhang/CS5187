...
def main(args):
    ...
    pretrain_path = args.pretrain_path  # 你的预训练权重路径
    pretrain_dict = torch.load(pretrain_path, map_location=device)

    model_dict = model.state_dict()
    pretrain_dict_filtered = {k: v for k, v in pretrain_dict.items() if 'coord_att' not in k}

    # 更新模型权重（仅更新CBAM+UNet部分）
    model_dict.update(pretrain_dict_filtered)
    model.load_state_dict(model_dict)
    print(f"成功加载预训练权重，共加载 {len(pretrain_dict_filtered)} 个参数")

    for name, param in model.named_parameters():
        if 'coord_att' in name:
            param.requires_grad = True  # 释放坐标注意力参数
            print(f"训练参数：{name}")
        else:
    ...
    print(f"可训练参数：{trainable_params} / 总参数：{total_params}（占比 {trainable_params/total_params:.2%}）")
...
def parse_args():
    ...
    parser.add_argument('--pretrain_path', type=str, default='./unet_lol_best.pth', help='path to pre-trained model weights')
